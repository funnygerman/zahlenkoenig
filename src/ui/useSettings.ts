...
// valid: an operator can't be deselected below one, and uniqueOnly turns
...
export function useSettings() {
  ...
  const toggleOp = useCallback((op: Operator) => {
    setSettings(s => {
      if (s.ops.includes(op) && s.ops.length <= 1) return s // concept 15.6: the last operator can't be deselected

...
| **Die Seite des **Ablegens** entscheidet, nicht die der Herkunft** (PO) | Sonst wäre die Hälfte der erreichbaren Ausdrücke (`(c × a + b) − d` aus `(a+b) × c − d`) nur über Auflösen und neu Umschließen zu haben. Die drei Beispiele des PO, in denen die Beschriftung „links"/„rechts" der eigenen Regel widerspricht, sind als Beschriftungsdreher gelesen: der Satz „wird auf der linken Seite abgelegt, landet links im Block" ist die Regel, die Beispiele illustrieren sie. |

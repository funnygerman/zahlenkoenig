...
//
// Superseded by the target-ranges-display round: that round's PO decision
// replaced tertile/operator-floor boundaries outright with a fixed policy
// (puzzles.ts's BandRow, generated by generateBandTable.ts's `multBands`) —
// one band per selection, except 3/4 numbers with × selected, which get
// fixed cut points at 50/100/250 regardless of any of the measurements
// below. This script's search (and its recommendations) describe the
// problem that led to that decision, not the mechanism generating today's
// table — kept for the historical record and because the exhaustive
// boundary search is still useful if a future round revisits per-selection
// cuts, not because anything here still drives BAND_TABLE.
//
// The PO's original report: "− , ÷ and [] will be used very seldom", with
...
//
// A band was then a tertile of target magnitude. With two single digits,
// − reaches at most 8 and ÷ at most 9, while × reaches 81 — so a tertile
// split puts − and ÷ entirely inside the lowest band and leaves the top
// band mathematically unable to contain them. This script measures that
// for every selection, and searches for the boundaries that would avoid
// it.
//
...
function selfTest(): void {
  ...
  // Known-good, against the live table: the target-ranges-display round's
  // fixed policy gives every 2-number selection a single band covering its
  // whole range — no boundary search runs for it any more, so this just
  // pins that fact down rather than re-deriving anything.
  const only = bandRanges(2, ALL_OPS)
  if (only.length !== 1) fail(`expected the 2-number selection to have one band, got ${only.length}`)
